...
async function seedDatabase() {
  try {
    ...
    if (achievementCount === 0) {
      ...
      console.log('Seeded default achievements successfully.');
    }

    const existingCertifications = await Certification.find();
    const hasHackathonOrOldMock = existingCertifications.some(c => 
      (c.image && c.image.includes('certificates/')) ||
      (c.title && (c.title.includes('CONNEXA') || c.title.includes('Kirti Avishkar') || c.title.includes('MSSU')))
    );

    if (existingCertifications.length === 0 || hasHackathonOrOldMock) {
      await Certification.deleteMany({});
      await Certification.insertMany(defaultCertifications);
      console.log('Seeded IBM / Coursera certifications successfully.');
    }
  } catch (error) {
